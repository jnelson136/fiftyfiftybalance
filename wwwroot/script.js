...
let connection = new signalR.HubConnectionBuilder()
    .withUrl("http://localhost:5136/clickHub")
    .build();
...
async function updateCounters() {
    let response = await fetch('http://localhost:5136/api/click');
...
document.getElementById('buttonA').addEventListener('click', async () => {
    await fetch('http://localhost:5136/api/click/clickA', { method: 'POST'});
    updateCounters();
});
...
document.getElementById('buttonB').addEventListener('click', async () => {
    await fetch('http://localhost:5136/api/click/clickB', { method: 'POST'});
    updateCounters();
});
